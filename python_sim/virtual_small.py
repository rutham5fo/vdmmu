...
class Manager:
    ...
    """
    def __init__(self, number, group_count=32):
        # Create internal regs
        self.num = number
        self.group_count = group_count
        self.avail_units = 0
        self.start_block = self.num*2
        self.free_group = 0
        self.offset = 0
    ...
    def run(self, left, right, groups_sel, groups_map):
        ...
        group = self.find_free_group(groups_map)
        ...
        #print(f'\n MANAGER [{self.num}] ||| RUN || Avail_units = {self.avail_units}, offset = {self.offset}, free_group = {self.free_group}')
        if (not self.req_q.empty()):
            # request = [req_type (0 = alloc, 1 = dealloc), req_size, init_offset, blkgrp, split]
            request = self.read_req()
            #print(f'\n MANAGER [{self.num}] ||| RUN || Received request = {request}')
            if (request != None):
                req_type = request[0]
                req_size = request[1]
                ...
                if (req_type == 1):
                    if (req_split):
                        ...
                        left_size = group_sel[0]
                        right_size = req_size-group_sel[0]
                    ...
                    if (req_size <= left_avail_units):
                        ...
                        send_left = 1
                        send_right = 1
                # Update Groups if split
                if (send_left == 1 and send_right == 1):
                    # Update groups_sel reg
                    groups_sel[fwd_group][0] = left_size
                    groups_sel[fwd_group][1] = right_offset
                    groups_map[fwd_group] = 1-req_type
            
                # Build return addr, iff this is the start node(manager)
                if (req_type == 0):
                    # ret_addr = [split (1), manum (1), blkgrp (5), offset (7), access_addr (9)]
                    if (send_left == 1 and send_right == 1):
                        ret_addr = [split, self.num, fwd_group, left_offset, 0]
                    elif(send_left == 1):
                        ret_addr = [split, self.num, self.start_block, left_offset, 0]
                    else:
                        ret_addr = [split, self.num, self.start_block+1, right_offset, 0]

            # Send request to left and right child
            if (send_left):
                left_req = [req_type, left_size, left_offset]
                left.write_req(left_req)
                #print(f'\n MANAGER [{self.num}] ||| RUN || Sending request = {left_req} to left child')
            if (send_right):
                right_req = [req_type, right_size, right_offset]
                right.write_req(right_req)
                #print(f'\n MANAGER [{self.num}] ||| RUN || Sending request = {right_req} to right child')
...
class virtualMem:
    ...
    def __init__(self, block_size, block_count, group_count):
        ...
        self.manager_count = int(self.block_count/2)
        self.group_count = group_count
        if (int(self.block_count%2) != 0):
            raise Exception(f'\n VIRTUAL ||| Block count ({self.block_count}) should be a multiple of 2')
        # Initialize Blocks
        self.blocks = [Block(self.block_size, x) for x in range(self.block_count)]
        # Initialize Block Managers
        self.managers = [Manager(x, self.group_count) for x in range(self.manager_count)]
        # Initialize Groups
        # Node_groups_sel = [size flag, offset] -> There can be max block_count-1 breaks in a group
        self.groups_sel = [[[0, 0] for _ in range(self.group_count)] for _ in range(self.manager_count)]
        self.groups_map = [[0 for _ in range(self.group_count)] for _ in range(self.manager_count)]
        # Initialize manager_orMap
        self.manorMap = [0 for x in range(self.manager_count)]
    ...
    def reset_elems(self):
        for y in range(2):
            for x in range(int(self.block_count/(2**y))):
                if (y == 0):
                    self.blocks[x].reset()
                else:
                    self.managers[x].reset()
    
    def run_elems(self):
        # direction = 0 -> Top-down, 1 -> bottom-up
        # first perform top-down to pass reqs
        # second perform bottom-up to update mans
        level_range = lambda dir: list(reversed(range(2))) if (dir == 0) else range(2)
        for d in range(2):
            ...
            for y in level_range(d):
                for x in range(int(self.block_count/(2**y))):
                    if (y == 0):
                        ...
                        self.managers[x].run(self.blocks[x*2], self.blocks[x*2+1], self.groups_sel[x], self.groups_map[x])
    
    def comp_orMap(self, request_size):
        comp_mans = lambda req_size, manum: 0 if (req_size <= self.managers[manum].get_avail_units()) else 1
        for x in range(self.manager_count):
            self.manorMap[x] = comp_mans(request_size, x)
    
    def find_start_node(self):
        ret_val = None
        for x in range(self.manager_count):
            if (self.manorMap[x] == 0):
                ret_val = x
                break
        return ret_val
    ...
    def alloc(self, request_size):
        ...
        manum = self.find_start_node()
        # Write to manager req_q
        # request = [req_type (0 = alloc, 1 = dealloc), req_size, init_offset, blkgrp, split]
        alloc_req = [0, request_size, 0, 0, 0]
        if (manum != None):
            #print(f'\n VIRTUAL ||| ALLOC || Sending request {alloc_req} to manager[{manum}]')
            self.managers[manum].write_req(alloc_req)
            # Update EMU metrics
    ...
    def dealloc(self, addr=None, size=0):
        ...
        blkgrp = addr[2]
        offset = addr[3]
        dealloc_req = [1, size, offset, blkgrp, split]
        #print(f'\n VIRTUAL ||| DEALLOC || Sending request {dealloc_req} to manager[{manum}]')
        self.managers[manum].write_req(dealloc_req)
    ...
    def show_groups(self):
        for y in range(self.group_count):
            group = []
            for x in range(self.manager_count):
                temp = [self.groups_map[x][y], self.groups_sel[x][y]]
                group.append(temp)
            print(f'\n VIRTUAL ||| GROUP [{y}] = {group}')
